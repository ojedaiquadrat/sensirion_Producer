...
async def main():
    ...
    async with BleakClient(second_sensor_mac_address) as bleSensorClient:
        ...
        with open("temperature_data.csv", "a") as temp_file, \
             open("co2_data.csv", "a") as co2_file, \
             open("humidity_data.csv", "a") as humidity_file:
            
            temp_writer = csv.writer(temp_file)
            temp_writer.writerow(["Timestamp", "Temperature (°C)"])  # Add header

            co2_writer = csv.writer(co2_file)
            co2_writer.writerow(["Timestamp", "CO2 (ppm)"])  # Add header

            humidity_writer = csv.writer(humidity_file)
            humidity_writer.writerow(["Timestamp", "Humidity (%)"])  # Add header
            ...
            while True:
                ...
                co2_concentration = int(getCO2())
                temperature_centigrades = int(getTemperature())
                humidity_percentage = int(getHumidity())

                # Get current timestamp
                timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")

                # Write data to separate CSV files
                temp_writer.writerow([timestamp, temperature_centigrades])
                co2_writer.writerow([timestamp, co2_concentration])
                humidity_writer.writerow([timestamp, humidity_percentage])
